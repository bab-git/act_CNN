...
def train(model, optimizer, loss_fn, dataloader, metrics, params,logger):
    ...
    with tqdm(total=len(dataloader)) as t:
        for i, (data_batch, labels_batch) in enumerate(dataloader):
            ...
            total_norm = torch.nn.tools.clip_grad_norm_(model.parameters(), params.clip*params.batch_size_train)
...
def evaluate(model, loss_fn, dataloader, metrics, params,logger):
    ...
    Args:
        ...
        dataloader: (DataLoader) a torch.tools.data.DataLoader object that fetches data
...
def train_and_evaluate(model, train_dataloader, val_dataloader, optimizer,
    ...
    if restore_file is not None:
        ...
        checkpoint = tools.load_checkpoint(restore_file, model, optimizer)
    ...
    for epoch in range(params.start_epoch,params.num_epochs):
        ...
        # Save weights
        tools.save_checkpoint( {'epoch': epoch + 1,
        ...
        if is_best:
            ...
            tools.save_dict_to_json(val_metrics, best_json_path)

        # Save latest val metrics in a json file in the model directory
        last_json_path = os.path.join(params.experiment_path, "metrics_val_last_weights.json")
        tools.save_dict_to_json(val_metrics, last_json_path)
...
if __name__ == '__main__':
    ...
    if params.mode =='train':
        tools.set_logger(os.path.join(experiment_path,'train.log'))
    elif params.mode =='test':
        tools.set_logger(os.path.join(experiment_path, 'test.log'))
    elif params.mode == 'load_train':
        tools.set_logger(os.path.join(experiment_path, 'load_train.log'))
